...
namespace Mirror
{
    ...
    class NetworkInformationPreview : ObjectPreview
    {
        ...
        // Styles is lazily initialized below to avoid accessing EditorStyles too early

        GUIContent title;
        // lazy-initialized to avoid accessing EditorStyles too early
        Styles styles;
        class Styles
        {
            ...
            public Styles()
            {
                // EditorStyles may not be available in some editor contexts; guard using try/catch
                GUIStyle baseLabel = null;
                GUIStyle baseBold = null;
                GUIStyle baseMini = null;
                try
                {
                    baseLabel = EditorStyles.label;
                    baseBold = EditorStyles.boldLabel;
                    baseMini = EditorStyles.miniLabel;
                }
                catch
                {
                    baseLabel = null;
                ...
                labelStyle = baseLabel != null ? new GUIStyle(baseLabel) : new GUIStyle();
                componentName = baseBold != null ? new GUIStyle(baseBold) : new GUIStyle();
                disabledName = baseMini != null ? new GUIStyle(baseMini) : new GUIStyle();
